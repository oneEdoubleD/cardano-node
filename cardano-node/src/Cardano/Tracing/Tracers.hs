...
instance ElidingTracer (WithSeverity (ChainDB.TraceEvent blk)) where
  ...
  conteliding tverb tr ev@(WithSeverity _ (ChainDB.TraceAddBlockEvent (ChainDB.AddedToCurrentChain pt _ _))) (_old, oldt) = do
      let slotno = toInteger $ unSlotNo (realPointSlot pt)
      tnow <- fromIntegral <$> getMonotonicTimeNSec
      let deltat = tnow - oldt
      when (deltat > 250000000 || slotno `mod` 200 == 0) $  -- report at most every 250 ms or every 200th block
